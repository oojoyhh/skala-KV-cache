...
| # | 담당 | 수정 가능한 파일 | 추가 지침 |
|---|---|---|---|
| 1 | RAG 파이프라인 | `rag/`, `data/papers/`, `tests/test_rag*.py` | `rag/retriever.py`에 `retrieve(query, k) -> list[Chunk]`, `paper_reference(arxiv_id) -> Reference`를 DEV_PLAN §3-2 형태로 구현. 논문 경로는 `config.PAPERS`, 청킹은 `config.CHUNK_SIZE`·`CHUNK_OVERLAP`, 검색은 `config.TOP_K`·`MMR_LAMBDA`. 임베딩은 `HuggingFaceEmbeddings(config.EMBEDDING_MODEL)`, 인덱스는 `config.FAISS_INDEX_DIR`에 캐시. PDF 로딩 실패는 E-1003 |
| 2 | 기술 조사 + 검색 평가 | `agents/research.py`, `eval/`, `prompts/research*.md`, `tests/test_research*.py` | `research_node`는 두 기술의 `tech_summary`와 `references` 반환. 관련성 체크 → 쿼리 재작성 루프는 노드 안에서 최대 `config.RAG_MAX_REWRITE`회. `retrieve`가 아직 없으면 같은 형태의 가짜 함수로 먼저 개발. `key_metrics` 수치는 검색된 청크에 있는 값만. `eval/retrieval_eval.py`는 `config.RETRIEVAL_EVAL_SET`으로 Hit Rate@5, MRR 출력 |
